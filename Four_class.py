...
def flicker_object(canvas, object, frequency, duration, object_positions, color, log_file, queue, cortex, ipc_queue):
    ...
    label_mapping = {"left": 0, "right": 1, "stop": 2, "rest": "Rest", "forward": 3}

    while time.time() < end_time:
        
        if object in ["left", "right", "forward"]:
...
def run_visual_stimuli(start_event, cortex, ipc_queue):
    try:
        ...
        object_positions = {
            ...
            "forward": ((width // 2) - arrow_size // 2, (height // 2) - arrow_size - button_size),
        ...
        with open(log_file_path, "w") as log_file, open(csv_file_path, "w", newline='') as csv_file:
            object_order = ["left", "right", "stop", "forward"]
            frequencies = [11, 13, 9, 15]
            duration = 14
            gap = 6
