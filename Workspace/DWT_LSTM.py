...
def compute_dwt_features(window_df):
    """
    Compute DWT features for a given window of EEG data.
    
    Parameters:
        - window_df: DataFrame containing EEG data for a single window
        
    Returns:
        - features: DataFrame containing DWT features
    """
    # Filter numeric columns only
    window_df = window_df.select_dtypes(include=[np.number])

    columns_to_add = []  # Initialize empty list to store new columns

    for col in window_df.columns:
        signal = window_df[col]
        coeffs = pywt.wavedec(signal, 'haar', level=4)  # Using Haar wavelet and decomposition up to level 4
        
        for i, coeff in enumerate(coeffs):
            col_name = f"{col}_dwt_{i}"
            new_col = pd.Series([np.mean(np.abs(coeff)), np.std(coeff)], name=col_name)
            columns_to_add.append(new_col)

    # Concatenate all new columns at once
    features = pd.concat(columns_to_add, axis=1).reset_index(drop=True)
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith(".csv") and filename not in processed_files:
            ...
            for i in range(num_windows):
                ...
                # Compute DWT features for this window
                dwt_features = compute_dwt_features(window_features)
                window_features = pd.concat([window_features.reset_index(drop=True), dwt_features.reset_index(drop=True)], axis=1)
