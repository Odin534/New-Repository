...
def compute_fft_features(window, fs=128):
    # Apply FFT along each channel
    yf = fft(window.values, axis=0)
    xf = fftfreq(window.shape[0], 1 / fs)[:window.shape[0] // 2]
    
    # Use the absolute values of the FFT coefficients as features
    fft_features = 2.0 / window.shape[0] * np.abs(yf[:window.shape[0] // 2])
    
    # Create a DataFrame from FFT features
    fft_df = pd.DataFrame(data=fft_features, columns=[f'fft_{i}' for i in range(fft_features.shape[1])])
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith(".csv") and filename not in processed_files:
            ...
            for i in range(num_windows):
                ...
                fft_features = compute_fft_features(window_features)
                window_features = pd.concat([window_features.reset_index(drop=True), fft_features.reset_index(drop=True)], axis=1)
